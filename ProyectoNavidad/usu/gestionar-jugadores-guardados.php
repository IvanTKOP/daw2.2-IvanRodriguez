...
if (isset($_REQUEST['agregar'])) {
    ...
    foreach ($equipo->getFichajes() as $fichaje) {
        if ($fichaje->getJugadorId() == $_REQUEST['jugadorId']) {
            DAO::jugadoresGuardadosVariarUnidades($_SESSION["id"], $_REQUEST['jugadorId'], $variacionUnidades);
...
if (isset($_REQUEST['eliminar'])) {
    $equipoId = DAO::jugadoresGuardadosVariarUnidades($_SESSION["id"], $_REQUEST["jugadorId"], 0);
